...
export default function Navbar() {
  ...
  const getLinkClass = (path) => {
    return location.pathname === path
      ? "text-green-400 font-bold transition-colors"
    ...
    switch (role) {
      case "restaurant":
        return (
          <>
            {commonLinks}
            <Link to="/ngo-requests" className={getLinkClass("/ngo-requests")}>
              Requests
            </Link>
            <Link
              to="/food-listings"
              className={getLinkClass("/food-listings")}
            >
              Food Listings
            </Link>
            <Link
              to="/restaurant-dashboard"
              className={getLinkClass("/restaurant-dashboard")}
            >
              Dashboard
            </Link>
            <UserPill />
        ...
        return (
          <>
            ...
            <Link
              ...
              className={getLinkClass("/manage-deliveries")}
            >
              Manage Deliveries
            </Link>
            <Link
              to="/ngo-dashboard"
              className={getLinkClass("/ngo-dashboard")}
            >
              Dashboard
            </Link>
            <UserPill />
        ...
        return (
          <>
            ...
            <Link
              ...
              className={getLinkClass("/volunteer-dashboard")}
            >
              Volunteer Hub
            </Link>
            <UserPill />
    ...
    <div className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl z-50">
      <nav className="relative flex justify-between items-center px-6 py-3 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
        <Link
          ...
          className="flex items-center gap-2 text-white font-black text-xl tracking-tight z-50"
        >
